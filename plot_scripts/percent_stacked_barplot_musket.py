...
fig.suptitle('Kernel Percentage - Musket ', fontsize=16, y=0.95)
...
for x_range in range(2):
    for y_range in range(2):    
        totals = np.array([i+j+k+l+m+n+o+p+q for i,j,k,l,m,n,o,p,q in zip(md[1+(9*setup_index)],md[2+(9*setup_index)],md[3+(9*setup_index)],md[4+(9*setup_index)],md[5+(9*setup_index)],md[6+(9*setup_index)],md[7+(9*setup_index)],md[8+(9*setup_index)],md[9+(9*setup_index)])])
        percents = np.zeros((9, 10))

        for x in range(9):
            percents[x] = np.array([i / j * 100 for i,j in zip(md[x+1+(9*setup_index)], totals)])
